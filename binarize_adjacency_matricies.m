...
calculate_connection_thresh = false
hist_plot_vis = 'off'
...
%             my_mkdir(dist_pth, sprintf('%s_%s_%s_%s_*',subj,ref,lock,band))
            ...
            for n = 1:length(f_subj_lock_band) % (11/27/22) What files are we looping through? A_deck should contain CMs for all events
                ...
                if calculate_connection_thresh
                    % Plot the Nonzero weight histogram with threshold displayed
                    figure('visible',hist_plot_vis)
                    hold on
