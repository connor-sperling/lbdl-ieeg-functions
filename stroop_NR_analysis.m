function stroop_NR_analysis(EEG, study, pth) 
    ...
    event_prep(EEG, evn, evn_idc, rtm, adat_pth, 'ALL') 
    ...
    for N = 1:4
        ...
        event_prep(ses_EEG, c_beg, c_beg_idcs, c_beg_resp, fdat_pth, ['c' cong stp 'Beg'])
        
        prompt('stroop evn prep', 4*(N-1)+2, 'End', 'c', cong, stp)
        event_prep(ses_EEG, c_end, c_end_idcs, c_end_resp, fdat_pth, ['c' cong stp 'End'])
        
        prompt('stroop evn prep', 4*(N-1)+3, 'Beg', 's', cong, stp)
        event_prep(ses_EEG, s_beg, s_beg_idcs, s_beg_resp, fdat_pth, ['s' cong stp 'Beg'])
        
        prompt('stroop evn prep', 4*(N-1)+4, 'End', 's', cong, stp)
        event_prep(ses_EEG, s_end, s_end_idcs, s_end_resp, fdat_pth, ['s' cong stp 'End'])
