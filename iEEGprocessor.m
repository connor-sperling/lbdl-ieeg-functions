...
if exist(old_file, 'file') && contains(fname, 'RAW')
    ...
    chnl_rej = {EEGold.reject.rej}';
    chnl_rej = chnl_rej(cellfun(@(x) ~isempty(x), chnl_rej));

    disp('These channels and events were previously rejected for this study')
    disp('Events:')
    disp(evrej_new)
    disp('  ')
    disp('Channels:')
    disp(chnl_rej)
    ke = user_yn('keep event rejection?');
    kc = user_yn('keep channel rejection?');

    if ke
        evn_msk = ~ismember(evn,evrej_new);
        an_evn_idc = evn_idc(evn_msk);
        an_evn = evn(evn_msk);
        an_rtm = rtm(evn_msk);
        a = make_EEG(EEG, 'AnalysisEventIdx', an_evn_idc, 'AnalysisEventType', an_evn, 'AnalysisResponseTime', an_rtm, 'EventIndex', evn_idc, 'EventType', evn, 'ResponseTime', rtm, 'EventReject', evrej_new);
        EEG = a;
    end

    if kc
        crej_no = find(ismember(glab, chnl_rej)==1);
        [gdat, glab, rdat, crej_lab] = remove_channels(gdat, glab, crej_no, [], {});
        EEG = make_EEG(EEG, 'dat', gdat, 'labels', glab, 'RejectChans', crej_lab, 'RejectChansData', rdat);
    end
